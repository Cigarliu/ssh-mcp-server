...
### **1. 源代码 ✅**
- **位置：** `c:\Users\test-user\Desktop\temp\code\ssh-mcp-server`
- **分支：** 当前分支
- **状态：** ✅ 所有更改已提交，编译通过
...
```json
{
  "mcpServers": {
    "ssh-mcp": {
      "command": "C:\\Users\\test-user\\Desktop\\temp\\code\\ssh-mcp-server\\bin\\sshmcp.exe",
      "args": []
    }
  }
}
...
```
连接到 [REDACTED_HOST]，用户 test-user，密码 [REDACTED_PASSWORD]，执行 ls 命令
```
