...
```bash
# 克隆仓库
git clone https://github.com/test-user/sshmcp.git
cd sshmcp
...
### 连接并执行命令
```
连接到 [REDACTED_HOST]，用户名 root，密码 root，执行命令 df -h
```
...
### 交互式 Shell
```
启动到 [REDACTED_HOST] 的交互式 shell，终端类型 xterm-256color，24行80列
```
...
[test-user](https://github.com/test-user)
