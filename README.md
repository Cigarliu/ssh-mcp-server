...
```
Connect to [REDACTED_HOST], username root, password root, execute ls -la
```
...
```
Start interactive shell to [REDACTED_HOST], terminal type xterm-256color, 24 rows, 80 columns
```
...
SSH_HOST=[REDACTED_HOST] SSH_USER=root SSH_PASSWORD=root go test ./pkg/sshmcp -v
...
[test-user](https://github.com/Cigarliu)
